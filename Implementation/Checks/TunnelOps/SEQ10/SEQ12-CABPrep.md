...
- Hardware or image corruption during migration
    - Mitigation: perform full system clone prior to change and validate clone integrity before live execution.

## Pre-Implementation Validation
- Prior to live implementation, perform a full disk clone of each system (`W9419`, `WT-Healthmon`, `W9316`) using approved imaging software.
- Execute all migration and remediation scripts against the cloned instance in an isolated test environment.
- Validate:
    - Shared account creation, logon restrictions, and RDP groups function as intended.
    - All key applications (WTTP, Matlab, Excel VBA link, Healthmonitor, Edge dashboards) launch and operate normally.
    - Network shares, paths, and local admin mappings resolve correctly.
- Obtain test sign-off from WT Systems before live execution.

## Execution Window & Live Cutover
- Live execution window: **02:00 – 06:00 a.m. (low-risk maintenance period)**.
- Prior to 02:00, ensure clone validation is complete and rollback image retained on secure media.
- During window:
    - Apply validated scripts and configuration changes to production systems.
    - Verify shared account logons and permissions in real time.
    - Conduct immediate functional test of WTTP, Healthmonitor, and network access.
- Post-window: confirm systems stable and log success in CAB closure notes.
